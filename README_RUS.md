...
6. [Глава VI](#глава-vi) \
    6.1. [Упражнение 02. Установка множества библиотек](#упражнение-02-установка-множества-библиотек)
...
Файлы для сдачи: venv.py и папка с вашим виртуальным окружением.
Разрешенные функции: import os.

Библиотеки, также известные как пакеты, — это один из способов, благодаря которому программирование стало более доступным. Никогда еще не было так легко учиться программировать и быстро видеть результаты. Некоторые программисты пишут код, который могут использовать другие. Многие из этих библиотек Python являются открытыми (open source), то есть каждый может ими пользоваться. Нет необходимости писать существующие классы, методы или функции с нуля, потому что их можно переиспользовать. Всё, что тебе нужно сделать, — это выполнить команду sudo pip install. Или погоди...
Такой способ установки пакетов Python считается плохой практикой. Когда ты устанавишь их, как описано выше, устанавливи их в системную версию Python. Python существует на твоей машине не только для того, чтобы дать возможность писать код, но и для запуска важных системных программ. Установка сторонних пакетов таким образом может нарушить работу системы. Тебе почти никогда не нужно использовать sudo pip install.

Лучший способ — использовать виртуальные окружения. Представьте, что это ваша собственная песочница, где вы можете делать всё, что захотите. Если вы что-то испортите, это затронет только эту песочницу. На твоей машине должен быть предустановлен пакет virtualenv. Если нет — свяжитесь с администраторами или установите его самостоятельно, если вы работаете на своем личном компьютере. Мы будем использовать его в следующих упражнениях и проектах.

Это упражнение простое и предназначено для разминки и знакомства с концепцией виртуальных окружений. Что нужно сделать:

1. Создать виртуальное окружение с вашим никнеймом в качестве имени, используя Python 3. Вы будете работать с этим окружением на протяжении всего курса.

2. Активировать его.

3. Запустить Python 3 из терминала.

4. Вывести название виртуального окружения с помощью библиотеки os.

5. Написать небольшой скрипт на Python, который делает это, запуская его из командной строки:
...
7. Запустить скрипт снова.

Если при деактивации окружения ты столкнешься с KeyError, значением None или каким-либо исключением, подумай, почему это произошло. Тебе не нужно исправлять это для данного упражнения, но будь готов объяснить, что случилось.
...
3. Создай для этого shell-скрипт с именем pies_bars.sh. В нём должна быть только часть, отвечающая за построение графика, **без** активации и деактивации окружения.
...
- **Попробуйте использовать другую HTTP-клиентскую библиотеку**, чтобы проверить, будет ли скрипт работать быстрее. Сохраните новый скрипт как financial_enhanced.py, а результаты профилирования - в файл profiling-http.txt.
...
- **Добавь тесты в код своего скрипта** financial.py**.** Сохрани файл в своей директории под именем financial_test.py. Запусти PyTest - все тесты должны быть пройдены. Если это не так, доработай скрипт, чтобы привести его в соответствие с требованиями.
